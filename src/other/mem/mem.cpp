...
#define ADD_PATTERN_REL( module_name, addr_name, pattern )                                                                       \
	m_modules[ module_name ].add_pattern( HASH_CT( addr_name ), STB( pattern ), true )
#define ADD_PATTERN( module_name, addr_name, pattern )                                                                           \
	m_modules[ module_name ].add_pattern( HASH_CT( addr_name ), STB( pattern ) )
#define ADD_ADDRESS( module_name, addr_name, address ) m_modules[ module_name ].add_address( HASH_CT( addr_name ), address )
	...
	{ // client.dll
		...
		{ // C_BasePlayer
			...
			ADD_PATTERN( CLIENT_DLL, "C_BasePlayer::SimulatePlayerSimulatedEntities",
		...
		{ // C_BaseAnimating
			...
			ADD_PATTERN( CLIENT_DLL, "C_BaseAnimating::ShouldSkipAnimationFrame",
		...
		{ // CCSGOPlayerAnimState
			...
			ADD_PATTERN_REL( CLIENT_DLL, "CCSGOPlayerAnimState::ModifyEyePosition",
			                 "E8 ? ? ? ? 8B 06 8B CE FF 90 ? ? ? ? 85 C0 74 50" );
			ADD_PATTERN_REL( CLIENT_DLL, "CCSGOPlayerAnimState::DoProceduralFootPlant",
			                 "E8 ? ? ? ? F6 86 ? ? ? ? ? 0F 84 ? ? ? ?" );
			ADD_PATTERN_REL( CLIENT_DLL, "CCSGOPlayerAnimState::SetUpMovement",
		...
		ADD_PATTERN( CLIENT_DLL, "C_CSPlayer::DoExtraBoneProcessing",
