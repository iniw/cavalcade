...
#ifndef LAZYIMPORTER_HPP
#define LAZYIMPORTER_HPP

#define LI_FN( name ) ::li::detail::lazy_function< ::li::detail::khash( #name ), decltype( &name ) >( )
...
namespace li {
	namespace detail {
		...
		LAZY_IMPORTER_FORCEINLINE hash_t::value_type hash( const win::UNICODE_STRING_T& str ) noexcept {
		...
		LAZY_IMPORTER_FORCEINLINE pair< hash_t::value_type, hash_t::value_type > hash_forwarded( const char* str ) noexcept {
			pair< hash_t::value_type, hash_t::value_type > module_and_function{ hash_t::offset, hash_t::offset };
							...
							while ( export_index-- )
								if ( hash( exports.name( export_index ) ) == function_hash ) {
									...
									if ( exports.is_forwarded( addr ) ) {
										auto hashes = hash_forwarded( reinterpret_cast< const char* >( addr ) );
