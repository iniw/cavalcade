...
//  2019/02/09: added RasterizerFlags::Monochrome flag to disable font anti-aliasing (combine with ::MonoHinting for best
...
bool ImFontAtlasBuildWithFreeTypeEx( FT_Library ft_library, ImFontAtlas* atlas, unsigned int extra_flags ) {
	...
	// 3. Unpack our bit map into a flat list (we now have all the Unicode points that we know are requested _and_ available _and_
	// not overlapping another)
	for ( int src_i = 0; src_i < src_tmp_array.Size; src_i++ ) {
		ImFontBuildSrcDataFT& src_tmp = src_tmp_array[ src_i ];
	...
	// We allocate in chunks of 256 KB to not waste too much extra memory ahead. Hopefully users of FreeType won't find the
	...
	// decrease height. User can override TexDesiredWidth and TexGlyphPadding if they wish, otherwise we use a simple heuristic to
	// select the width based on expected surface.
