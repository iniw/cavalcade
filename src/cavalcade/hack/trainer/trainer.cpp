...
void hack::trainer::go( ) {
	...
	g_csgo.m_client_mode_shared->m_chat_element->chat_printf(
		0, 0, "<<<NO_TRANSLATE>>> [<font color=\"#FF0000\">TRAINER</font>] Teleported to checkpoint #%d...", m_entry + 1 );
}
...
void hack::trainer::run( ) {
	if ( g_io.key_state< io::key_state::RELEASED >( VK_XBUTTON1 ) ) {
		...
		if ( ( m_entry + 1 ) < m_checkpoints.size( ) ) {
			...
			g_csgo.m_client_mode_shared->m_chat_element->chat_printf(
				0, 0, "<<<NO_TRANSLATE>>> [<font color=\"#FF0000\">TRAINER</font>] Not enough entries to update focus forward..." );
		}
		...
		if ( !m_checkpoints.empty( ) ) {
			g_csgo.m_client_mode_shared->m_chat_element->chat_printf(
				0, 0, "<<<NO_TRANSLATE>>> [<font color=\"#FF0000\">TRAINER</font>] Erased entry that was formerly #%d", m_entry + 1 );
			...
			g_csgo.m_client_mode_shared->m_chat_element->chat_printf(
				0, 0, "<<<NO_TRANSLATE>>> [<font color=\"#FF0000\">TRAINER</font>] No entries to erase..." );
		}
	}
}
