...
void hack::other::prediction::start( ) {
	...
	g_csgo.m_globals->m_curtime   = g_ctx.ticks_to_time( g_ctx.m_local->get_tickbase( ) );
	...
	if ( g_ctx.m_cmd->m_weapon_select ) {
		auto weapon = g_csgo.m_entity_list->get< sdk::weapon_cs_base* >( g_ctx.m_cmd->m_weapon_select );
		if ( weapon ) {
