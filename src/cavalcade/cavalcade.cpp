...
DWORD WINAPI cavalcade::init( unk module_handle ) {
	...
	//                --        --g_ConVars:ConsolePrint('Head pos x ' .. enemy_head_pos.m_X .. ' y ' .. enemy_head_pos.m_Y .. ' z ' ..
	//                --enemy_head_pos.m_Z)

	//                --        --trace = g_EngineTrace:TraceRay(Ray.new(local_eye_pos, enemy_head_pos), 0x46004009,
	//                TraceFilter.new--(g_Local:GetRef()))
